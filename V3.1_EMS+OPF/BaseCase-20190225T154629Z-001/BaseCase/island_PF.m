% Power Flow di reti elettriche in isola, partendo dai parametri di rete,
% generatori e carichi
% 
%   results=island_PF(netdata,RegG,RegL)
% 
...
% - results.w  - Frequenza di regime della rete [rad/s]
% 
% Gli input sono:
% - dati della rete
...
%     netdata.Vs          - tensione nominale del nodo di riferimento [V]
%     netdata.wn          - frequenza nominale della rete [rad/s]
%     netdata.R/L/C/G/T/k - resistenze/induttanze/capacità/conduttanze/
%                           elastanze/rapporti di trasformazione dei rami
%                           di rete [Ohm / H / F / 1/Ohm / 1/H / V/V]
%                           (se non dichiarati R,L,C,G,T vengono assunti
%                           nulli, mentre k viene imposto uguale a 1)
% 
...
%                        convergenza. Di default si ha Tol=[1e-5,1e-5,0.10]
%                        per cui il sistema si ritiene a convergenza quando
%                        la variazione di tensione/frequenza/potenza tra
%                        due iterazioni successive è inferiore a
%                           0.001%    variazione modulo di tensione
%                           0.001%    variazione frequenza
...
%                        rete a vuoto [V]
% - options.win        - Valore iniziale della frequenza per la prima
%                        iterazione di Power Flow. Di deafult viene
%                        selezionato un valore in cui vi regolazione da
%                        parte dei generatori e/o autoregolazione dei
%                        carichi [rad/s]
% 
% La funzione restituisce dettagli riguardo al processo di Power flow. In
% particolare:
% - details.dv/dw/ds     - valori dei residui di tensione/frequenza/potenza
%                          per ogni step del Power Flow
