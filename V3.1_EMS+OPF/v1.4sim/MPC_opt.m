 function y = MPC_opt(u)


x=u(1:end-5);
Umpc_0= u(end-4:end-1);
t = u(end)+1;
...
%%
var_D = var_dist(:,(t):(t+N-1));

[J] = RealJacobian([1;x(1:n_nodes-1)], [0;x(n_nodes:2*n_nodes-2)], netdata);
Jinv  =  inv(J);
[Jloss, Ploss] = LossesJacobian([1;x(1:n_nodes-1)], [0;x(n_nodes:2*n_nodes-2)], netdata);
...
J_V    = 1e3*MPCpar.Cf.p_v(2)*(sum(h_v_stp(:))); % ( MPCpar.Cf.p_v(2).*(var_X_V_2*var_X_V_2') + MPCpar.Cf.p_v(3).*(var_X_V_3*var_X_V_3') )+ MPCpar.Cf.p_v(4)*(var_X_V_4*var_X_V_4') +  MPCpar.Cf.p_v(5)*(var_X_V_5*var_X_V_5');
...
SOC = [double(X_SOC_pq_1(1)); double(X_SOC_pq_2(1)); double(X_SOC_s(2))];

PQref = [double(X_Pbatt_1(1)),double(X_Pbatt_2(1)),double(X_Qbatt_1(1)),double(X_Qbatt_2(1))]';
